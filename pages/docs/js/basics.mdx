...
A **Promise** is an object returned by an a synchronous function, which represents the current state of the operation. It have two properties: `state` and `result`.
...
```javascript
new Promise((resolve, rejected) => {}).then((result) => {console.log("result", result)})
```
