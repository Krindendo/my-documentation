...
<Callout type="danger">
  Prior to React 19, exposing a ref from a function component required
  wrapping it in `forwardRef`. As of React 19, `forwardRef` is deprecated 
  `ref` can be received as a regular prop instead, as shown below. See
  [forwardRef](/docs/react/apis#forwardref) for the old-vs-new comparison.
</Callout>
...
```tsx title="MyInput.tsx" {6-11}
import { useImperativeHandle, useRef } from "react";

function MyInput({
  ref,
  ...props
}: React.ComponentProps<"input"> & { ref?: React.Ref<{ focus: () => void; scrollIntoView: () => void }> }) {
