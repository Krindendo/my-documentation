...
import { TimerButton } from "@/components/TimerButton";
import { StoreProvider } from "@/components/TimerProvider";
import { TimerView } from "@/components/TimerView";
...
export const TimerView = observer(() => {
  ...
  return <p>Seconds passed: {timer.secondsPassed}</p>;
});
...
<Callout type="warning">
  Both `TimerView` and `TimerButton` read the *same* store instance through
  `useSecoundsPassed()` from context  that's what keeps them in sync.
  Creating a second, separate `observable(...)` elsewhere (e.g. with its own
  `useState`) and passing it down as a prop would give you two independent
  stores that don't share updates.
</Callout>
