...
`key` is a special prop React uses to identify which items in a list have changed, been added, or been removed  it isn't passed down to your component as a real prop. Keys should be stable and unique among siblings (usually an ID from your data), **not** the array index  using the index breaks when items get reordered, inserted, or removed, since React can end up matching the wrong DOM node or state to the wrong item.
...
- **Uncontrolled errors**: Errors that aren't caught anywhere and crash (or partially crash) the app  for example, a runtime error thrown during render that isn't wrapped in an error boundary. The user is left with a broken UI or a blank screen.

- **Controlled errors**: Errors that are explicitly caught and handled, so the app can recover gracefully. In React, this is done with an **Error Boundary**  a class component that implements `static getDerivedStateFromError()` and/or `componentDidCatch()` to catch errors thrown by its child tree during render, and show a fallback UI instead of crashing.
...
Error boundaries only catch errors during **rendering**, in lifecycle methods, and in constructors of the tree below them  they do **not** catch errors in event handlers, async code, or errors thrown in the boundary itself. For those, use a regular `try`/`catch` (see [Error Handling](/docs/js/error-handling)).
...
The stages below are the traditional **class component** lifecycle. Function components (the modern default) don't have these named methods  `useEffect` covers mounting, updating, and unmounting in a single Hook, matching a dependency array instead of separate method names. See [Effect Hooks](/docs/react/effect-hooks) for the Hook-based equivalent.
